...
-- contra tariff_annex_products.codigo_producto (misma normalización técnica).
...
-- Dimensión de validación del Anexo Tarifario por ítem (ADR-009: estados
-- ortogonales; no se colapsa en una columna de estado general).
...
-- futuras y las revalidaciones aplican la regla completa (ADR-024 §6).
...
ALTER TABLE "authorization_items" ADD CONSTRAINT "authorization_items_ready_prerequisites_check" CHECK ("authorization_items"."operation_status" IS NULL OR "authorization_items"."operation_status" <> 'READY_TO_DISPENSE' OR (
  ...
  "authorization_items"."tariff_membership_status" = 'LISTED' AND (
...
-- Permisos atómicos del Anexo Tarifario. Solo la organización MTD administra;
-- MTD_ADMIN los recibe por asignación explícita (el cross join de 0010 ya se
-- ejecutó antes de que estos permisos existieran).
