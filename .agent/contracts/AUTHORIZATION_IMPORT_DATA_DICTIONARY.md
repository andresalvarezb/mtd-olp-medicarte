...
**Version 3:** se agregan `CPRG` y `CDGN001` como columnas de evidencia. `CPRG` se conserva en el cargue, pero no se expone en descargas posteriores; `CDGN001` sí se expone después de `NOMBRE_PACIENTE`.
...
| `CPRG`                   | Escalar de origen     |          No | Ninguna; preservar evidencia           | Se conserva en `source_data`, pero se omite de las descargas posteriores.                                                                                                                              |
| `CDGN001`                | Escalar de origen     |          No | Ninguna; preservar evidencia           | Se conserva en `source_data` y se incluye en las descargas inmediatamente después de `NOMBRE_PACIENTE`.                                                                                                |
...
| `ESTADO_AUTORIZACION`    | Escalar textualizable |          Si | `normalizar`                           | No vacio; `5` produce `HABILITADO`; cualquier otro valor produce `BLOQUEADO_POR_ESTADO_ORIGEN` y no rechaza la fila.                                                                                  |
| `No.PRESCRIPCION`        | Escalar textualizable |          Si | `normalizar`; vacio es valido          | Encabezado obligatorio; valor opcional. Vacio produce `PBS`. No vacio: solo digitos y longitud mayor a 3, produce `NO_PBS`; en caso contrario `INVALID_FIELD_FORMAT`. Ver derivacion `no_prescripcion`. |
...
Para una llave repetida dentro del archivo, la primera aparicion conserva su resultado normal y la segunda aparicion y posteriores producen `DUPLICATE_IN_FILE`. Una llave existente produce `EXISTING_ITEM_REVIEW_REQUIRED` y no se actualiza automaticamente. La actualizacion explicita solo se permite en `LISTO_PARA_DISPENSAR`, reemplaza la evidencia y reevalua las cuatro columnas de negocio (`NUMERO_AUTORIZACION`, `COD_COMERCIAL`, `ESTADO_AUTORIZACION`, `No.PRESCRIPCION`), recalcula `operation_status` y se bloquea desde `DISPENSACION_REPORTADA`.

La actualizacion conserva `LISTO_PARA_DISPENSAR` solo para `HABILITADO + PBS + NO_APLICA` o `HABILITADO + NO_PBS + CONFIRMADO`; cualquier otra combinacion queda `BLOQUEADO`. La pareja normalizada `NUMERO_AUTORIZACION + COD_COMERCIAL` debe coincidir con la llave existente y sus componentes no cambian. En Fase 2, `NO_PBS + HABILITADO + PENDIENTE` queda bloqueado sin consulta externa.
...
- No agregar obligatoriedad o reglas semanticas a las 24 columnas de evidencia sin una decision documentada.
