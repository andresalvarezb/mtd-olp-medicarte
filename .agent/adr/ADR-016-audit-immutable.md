...
Para una actualización explícita F2, el evento referencia las filas de importación anterior y nueva, conserva hashes de su evidencia, compara `NUMERO_AUTORIZACION`, `COD_COMERCIAL`, `ESTADO_AUTORIZACION` y `No.PRESCRIPCION` normalizados antes/después, y enlaza el registro idempotente de la misma transacción. La evidencia cruda permanece en las tablas de negocio y no se duplica en auditoría ni en la respuesta idempotente persistida.
