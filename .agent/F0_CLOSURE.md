...
| Diccionario del archivo                     | ACCEPTED | `contracts/AUTHORIZATION_IMPORT_DATA_DICTIONARY.md`, SPEC-001 | 28 columnas; `CPRG` se conserva sin descarga; `CDGN001` se descarga tras `NOMBRE_PACIENTE`; negocio: llave, `ESTADO_AUTORIZACION`, `No.PRESCRIPCION`. |
...
| Actualizacion de llave existente            | ACCEPTED | DEC-002, DEC-014, ADR-021                                     | Revision humana; solo en `LISTO_PARA_DISPENSAR`; recalcula el estado operacional. |
...
| Dispensacion reportada/aprobada             | ACCEPTED | DEC-003, DEC-004, ADR-009                                     | OLP reporta fecha; `DISPENSADO` solo despues de `APROBADO`.                    |
...
| Fechas para iniciar auditoria               | ACCEPTED | DEC-015, SPEC-002, SPEC-006                                   | Ambas fechas producen LISTO; nunca APROBADO automatico.                       |
