...
export class DeliveryRepository {
  ...
  async create(body: CreateDeliveryRequest, scope: Scope) {
    return this.database.db.transaction(async (tx) => {
      ...
      if (
        !order.rows[0] ||
        !['ACCEPTED', 'PARTIALLY_ACCEPTED', 'IN_FULFILLMENT', 'PARTIALLY_DISPATCHED'].includes(
          order.rows[0].status,
        )
      )
        throw new Error('DELIVERY_ORDER_NOT_DELIVERABLE');
      const inserted = await tx.execute<{ id: string }>(
        sql`insert into deliveries (purchase_order_id, supplier_reference, created_by, updated_by) values (${body.purchaseOrderId}, ${body.supplierReference ?? null}, ${scope.userId}, ${scope.userId}) returning id`,
      );
      const id = inserted.rows[0]!.id;
      await this.insertLines(tx, id, body.lines);
      await tx.execute(
        sql`update purchase_orders set status = case when status in ('ACCEPTED', 'PARTIALLY_ACCEPTED') then 'IN_FULFILLMENT' else status end, updated_at = now(), updated_by = ${scope.userId} where id = ${body.purchaseOrderId}`,
      );
  ...
  async update(
    ...
    return this.database.db.transaction(async (tx) => {
      const delivery = await tx.execute<{
        purchase_order_id: string;
        version: number;
        status: string;
      }>(
        sql`select purchase_order_id, version, status from deliveries where id = ${id} for update`,
      );
      ...
      await this.insertLines(tx, id, body.lines);
      await tx.execute(
        sql`update deliveries set supplier_reference = coalesce(${body.supplierReference ?? null}, supplier_reference), version = version + 1, updated_at = now(), updated_by = ${scope.userId} where id = ${id}`,
      );
  ...
  async dispatch(
    ...
    return this.database.db.transaction(async (tx) => {
      ...
      const lines = await tx.execute<{
        ...
        sql`select purchase_order_line_id, quantity, commercial_code, dispensing_point_id, lot_number, expiration_date from delivery_lines where delivery_id = ${id} order by id`,
      );
      if (
        !lines.rows.length ||
        lines.rows.some((line) => !line.lot_number || !line.expiration_date)
      )
        throw new Error('DELIVERY_LOT_EXPIRATION_REQUIRED');
      ...
      for (const line of lines.rows) {
        ...
        requestedByLine.set(
          ...
          (requestedByLine.get(line.purchase_order_line_id) ?? 0) + line.quantity,
        );
        const requestedInDelivery = requestedByLine.get(line.purchase_order_line_id)!;
        const dispatched = await tx.execute<{ quantity: number }>(
          sql`select coalesce(sum(dl.quantity), 0)::int quantity from delivery_lines dl join deliveries d on d.id = dl.delivery_id where dl.purchase_order_line_id = ${line.purchase_order_line_id} and d.status in ('DISPATCHED','RECEIVED')`,
        );
        if (requestedInDelivery > orderLine.acceptedQuantity - (dispatched.rows[0]?.quantity ?? 0))
          throw new Error('DELIVERY_OVER_DISPATCHED');
        if (
          line.commercial_code !== orderLine.commercialCode ||
          line.dispensing_point_id !== orderLine.dispensingPointId
        )
  ...
  async cancel(id: string, expectedVersion: number, scope: Scope) {
    return this.database.db.transaction(async (tx) => {
      const result = await tx.execute<{ version: number; status: string }>(
        sql`select version, status from deliveries where id = ${id} for update`,
      );
      ...
      await tx.execute(
        sql`update deliveries set status = 'CANCELLED', version = version + 1, updated_at = now(), updated_by = ${scope.userId} where id = ${id}`,
      );
  ...
  async list(scope: Scope) {
    const rows = await this.database.db.execute<{ id: string }>(
      sql`select distinct d.id, d.created_at from deliveries d join delivery_lines dl on dl.delivery_id = d.id join dispensing_points dp on dp.id = dl.dispensing_point_id where ${this.scopeFilter(scope)} order by d.created_at desc`,
    );
    return Promise.all(rows.rows.map((row) => this.findById(row.id, scope)));
  }

  findById(id: string, scope: Scope) {
    return this.findByIdOn(this.database.db, id, scope);
  }
  ...
  private async findByIdOn(conn: Tx | Database['db'], id: string, scope: Scope) {
    ...
    return {
      ...
      lines: rows.rows.map((line) => ({
        ...
        dispensingPointName: line.dispensing_point_name,
  ...
  private async lockOrderLines(
    ...
    const rows = await tx.execute<{
      ...
      sql`select pol.id, pol.accepted_quantity, pol.commercial_code, pol.dispensing_point_id from purchase_order_lines pol join purchase_orders po on po.id = pol.purchase_order_id join dispensing_points dp on dp.id = pol.dispensing_point_id where pol.purchase_order_id = ${orderId} and pol.id in (${sql.join(
        lineIds.map((id) => sql`${id}`),
        sql`, `,
      )}) and ${this.scopeFilter(scope)} order by pol.id for update of pol`,
    );
    if (rows.rows.length !== new Set(lineIds).size) throw new Error('DELIVERY_LINE_OUT_OF_SCOPE');
    if (rows.rows.some((line) => line.accepted_quantity === null || line.accepted_quantity <= 0))
      throw new Error('DELIVERY_LINE_NOT_ACCEPTED');
  ...
  private async deriveOrderStatus(tx: Tx, orderId: string, userId: string) {
    ...
    await tx.execute(
      sql`update purchase_orders set status = ${status}, updated_at = now(), updated_by = ${userId} where id = ${orderId}`,
    );
  }

  private scopeFilter(scope: Scope) {
    const org =
      scope.organizationCode === 'OLP' || scope.organizationCode === 'MEDICARTE'
        ? sql`true`
