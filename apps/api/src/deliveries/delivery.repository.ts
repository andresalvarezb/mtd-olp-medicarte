  ...
  async dispatch(id: string, expectedVersion: number, scope: Scope): Promise<Outcome<Awaited<ReturnType<DeliveryRepository['findById']>>>> {
    return this.database.db.transaction(async (tx) => {
      ...
      for (const line of lines.rows) {
        ...
        const dispatched = await tx.execute<{ quantity: number }>(sql`select coalesce(sum(dl.quantity), 0)::int quantity from delivery_lines dl join deliveries d on d.id = dl.delivery_id where dl.purchase_order_line_id = ${line.purchase_order_line_id} and d.status in ('DISPATCHED','RECEIVED')`);
