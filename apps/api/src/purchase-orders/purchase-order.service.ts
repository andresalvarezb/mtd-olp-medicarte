...
export class PurchaseOrderService {
  ...
  create(body: CreatePurchaseOrderRequest, actor: Scope) { return this.run(() => this.repository.create({ body, actor: this.actor(actor) })); }
  ...
  available(periodId: string) { return this.repository.available(periodId); }
  private actor(scope: Scope) { return { userId: scope.userId, organizationId: scope.organizationId, correlationId: scope.correlationId }; }
  private async run<T>(action: () => Promise<T>): Promise<T> { try { const result = await action(); if (result && typeof result === 'object' && 'outcome' in result) { const outcome = result as { outcome: string; currentVersion?: number }; if (outcome.outcome === 'not_found') throw new NotFoundException({ code: 'PURCHASE_ORDER_NOT_FOUND', message: 'Purchase order not found' }); if (outcome.outcome === 'version_conflict') throw new ConflictException({ code: 'VERSION_CONFLICT', message: 'Purchase order changed during the operation', fields: { currentVersion: [String(outcome.currentVersion)] } }); } return result; } catch (error) { if (error instanceof ConflictException || error instanceof NotFoundException) throw error; const message = error instanceof Error ? error.message : ''; const codes: Record<string, [number, string]> = { PURCHASE_ORDER_CODE_REQUIRED: [409, 'Purchase order code is required before issue'], PURCHASE_ORDER_FROZEN: [409, 'Issued purchase orders are frozen'], PURCHASE_ORDER_DEMAND_OVERALLOCATED: [409, 'Demand is no longer available'], PROJECTED_DEMAND_REVISION_CONFLICT: [409, 'Projected demand revision changed'], PURCHASE_ORDER_BUCKET_MISMATCH: [400, 'Order type and demand bucket do not match'], PURCHASE_ORDER_INVALID_TRANSITION: [409, 'Invalid purchase order transition'], TARIFF_RATE_NOT_FOUND: [400, 'No active COMPENSAR tariff exists for the product'], PURCHASE_ORDER_NOT_REVIEWABLE: [409, 'Purchase order is not under supplier review'], PURCHASE_ORDER_REVIEW_INCOMPLETE: [409, 'All lines must be reviewed'] }; const [status, text] = codes[message] ?? [400, message || 'Invalid purchase order']; throw status === 409 ? new ConflictException({ code: message, message: text }) : new BadRequestException({ code: message, message: text }); } }
}
