...
export class PurchaseOrderController {
  ...
  @Get('purchase-orders') async list(@Query() raw: unknown, @Headers('x-organization-id') org: string | undefined, @Req() req: AuthenticatedRequest) { const scope = await this.scope(req, org, 'purchase_orders.read'); return { items: await this.orders.list(purchaseOrderListQuerySchema.parse(raw ?? {}), scope) }; }
...
export class SupplierPurchaseOrderController {
  ...
  @Get('purchase-orders') async list(@Query() raw: unknown, @Headers('x-organization-id') org: string | undefined, @Req() req: AuthenticatedRequest) { const scope = await this.scope(req, org, 'purchase_orders.read'); return { items: await this.orders.list(purchaseOrderListQuerySchema.parse(raw ?? {}), scope, true) }; }
