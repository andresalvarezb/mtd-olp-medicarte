...
const authorizationItemResponseSchema = {
  ...
  properties: {
    ...
    enablementStatus: {
      type: 'string',
      enum: ['HABILITADO', 'BLOQUEADO_POR_ESTADO_ORIGEN'],
    },
    coverageType: { type: 'string' },
    directionStatus: {
      type: 'string',
      enum: ['NO_APLICA', 'PENDIENTE', 'CONFIRMADO', 'ERROR_DE_CONSULTA'],
    },
    operationStatus: {
      type: 'string',
      enum: ['BLOQUEADO', 'LISTO_PARA_DISPENSAR', 'DISPENSACION_REPORTADA', 'DISPENSADO', 'VENCIDO'],
      nullable: true,
    },
    ...
    applicationSiteStatus: { type: 'string', enum: ['PENDIENTE_ASIGNACION', 'ASIGNADO'] },
    operationalVersion: { type: 'integer' },
    coverageRuleVersion: { type: 'string' },
    tariffMembershipStatus: {
      type: 'string',
       enum: ['NO_EVALUADO', 'LISTADO', 'NO_LISTADO'],
    },
