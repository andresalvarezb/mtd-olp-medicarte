...
describe('Gate ESP-003 — programación de pacientes', () => {
  ...
  it('25. no se crea orden de compra ni inventario', async () => {
    const tables = await database.query<{ table_name: string }>(
      `select table_name from information_schema.tables
        where table_schema = 'public'
          and table_name in ('purchase_orders', 'purchase_order_lines', 'inventory',
                             'inventory_items', 'inventory_stock', 'deliveries', 'receipts')`,
    );
