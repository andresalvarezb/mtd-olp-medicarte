...
function locationCsv(
  rows: Array<{ authorizationKey: string; location: string; scheduledDate?: string }>,
): string {
  return [
    'authorization_key,lugar_dispensacion,fecha_programada',
    ...rows.map((row) =>
      csvRow([row.authorizationKey, row.location, row.scheduledDate ?? '2026-10-01']),
    ),
    '',
  ].join('\n');
}
...
describe('Gate F4', () => {
  ...
  it(
    ...
    async () => {
      ...
      const extraColumns = await createBulkBatch(
        ...
        `authorization_key,lugar_dispensacion,fecha_programada,extra\n${authorizationKey},Calle 1,2026-10-01,X\n`,
      );
      ...
      const afterFirst = await database.query<{
        ...
        'select lugar_dispensacion, fecha_programada::text, operational_version from authorization_items where id = $1',
        [itemId],
      );
      ...
      const changes = await database.query<{
        ...
        `select field_name, previous_value, new_value, previous_operational_version, new_operational_version,
      ...
      expect(changes.rows[2]).toMatchObject({
        field_name: 'LUGAR_DISPENSACION',
      ...
      const locationNotification = await database.query<{
        id: string;
  ...
  it(
    ...
    async () => {
      ...
      const batch = await createBulkBatch(
        ...
        [
          'authorization_key,lugar_dispensacion,fecha_programada',
          csvRow([readyKey, '   ', '2026-10-01']),
          csvRow([readyKey, 'Calle valida 1', '2026-10-01']),
          csvRow(['FUERA:LEJA', 'Calle fuera de alcance', '2026-10-01']),
          csvRow(['', 'Sin llave', '2026-10-01']),
          '',
        ].join('\n'),
      );
  ...
  it(
    'mantiene el valor persistido aunque la entrega falle y permite reintentar desde la bandeja',
    { timeout: 90_000 },
    async () => {
      ...
      const recipientList = await fetch(
        `${apiUrl}/api/v1/admin/notification-recipients?notificationType=DISPENSATION_LOCATION_ASSIGNED`,
        {
          headers: {
            authorization: `Bearer ${adminToken}`,
            'x-organization-id': mtdOrganizationId,
  ...
  it(
    'expone el lugar y el estado derivado en lectura y permite la descarga completa de MEDICARTE',
    { timeout: 60_000 },
    async () => {
      ...
      expect(csv).toContain('ESTADO_PUNTO_APLICACION');
