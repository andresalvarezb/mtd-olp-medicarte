  ...
  it('persists prepared preview without publishing or changing catalog', async () => {
    ...
    await database.query(
      `insert into tariff_annex_imports
         (id,organization_id,created_by,original_filename,mime_type,size_bytes,sha256,status,
          correlation_id,idempotency_key,preview,preview_total,preview_unchanged,preview_changed,
          preview_anomalous,preview_rejected,preview_scale_pattern_detected)
       values
         ($1,$2,$3,'e2e.xlsx',
          'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
          1,$4,'PREPARED',$5,$6,$7::jsonb,1,0,1,0,0,false)`,
      [
        batchId,
    ...
    await database.query(
      `insert into tariff_annex_import_source_files
         (id,import_id,original_filename,mime_type,size_bytes,sha256)
       values
         ($1,$2,'e2e.xlsx',
          'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
          1,$3)`,
  ...
  it('enforces anomaly override, outbox idempotency and append-only history', async () => {
    ...
    const product = await database.query<{ id: string }>(
      `select id
       ...
       where codigo_producto='12690'`,
    );

    const auth = await database.query<{ id: string }>(
      'select id from authorization_items limit 1',
    );
    ...
    await database.query(
      `insert into tariff_annex_imports
         (id,organization_id,created_by,original_filename,mime_type,size_bytes,sha256,status,
          correlation_id,idempotency_key,preview,preview_total,preview_anomalous)
       values
         ($1,$2,$3,'anomaly.xlsx','application/octet-stream',1,$4,'PREPARED',
          $5,$6,$7::jsonb,1,1)`,
      [
        batchId,
        organization.rows[0].id,
    ...
    await expect(
      database.query(
        `update tariff_annex_imports
         ...
         where id=$1 and preview_anomalous=0`,
        [batchId],
      ),
    ).resolves.toBeTruthy();
    ...
    await database.query(
      `update tariff_annex_imports
       set status='CONFIRMING',
           confirmed_at=now(),
           confirmed_by=$2,
           override_reason='E2E explicit correction'
       where id=$1 and preview_anomalous>0`,
      [batchId, user.rows[0].id],
    );
    ...
    await database.query(
      `insert into outbox_events
       ...
       values
         ($1,'tariff.import',1,$2::jsonb,$3,$4,$5)
       on conflict (idempotency_key) do nothing`,
      [
        eventId,
        JSON.stringify({
          batchId,
          overrideReason: 'E2E explicit correction',
        }),
        correlationId,
        organization.rows[0].id,
    ...
    await expect(
      database.query(
        `update tariff_product_revisions
    ...
    await expect(
      database.query(
        'delete from tariff_product_revisions where id=$1',
        [revision.rows[0].id],
      ),
    ).rejects.toThrow('append-only');
    ...
    await database.query(
      `insert into authorization_tariff_snapshots
         (authorization_item_id,product_id,product_revision_id,codigo_producto,status,
          tarifa_unidad_raw,tarifa_unidad_canonical,provenance)
       values
         ($1,$2,$3,'12690','RESOLVED','7420',7420,'E2E')
